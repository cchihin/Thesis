...
We also note a transitional regime in which both states coexist.
...
Nonetheless, the transition to turbulence is subcritical in shear flow configurations, meaning that the route-of-chaos scenarios do not always occur via bifurcations from the laminar state.
...
A family of equilibrium and travelling-wave solutions for plane Couette and plane Poiseuille flows under different boundary conditions (i.e., stress-free, slip, and no-slip) was later identified by \citet{waleffe_exact_2001} and \citet{waleffe_homotopy_2003}.
...
% While these unstable solutions demonstrate good agreement with results from DNS such as the spanwise length scales, and mean and fluctuations, they do not capture the dynamical processes.
...
Invariant solutions such as equilibria, travelling waves, periodic orbits, and relative periodic orbits provide a building-block description of turbulence.
...
\begin{figure}[h]
    ...
    \caption{A graphical representation of the edge (grey surface) separating the basins of attraction of the laminar and turbulent attractor, consisting of attractors, known as edge states, adapted from \protect\citet{graham_exact_2021}.}
...
A consequence of this is that the laminar and turbulent states form bistable attractors in phase space with a boundary, separating their respective basins of attraction, known as the basin boundary.
Attractors that sit along this basin boundary have been identified and found to possess a saddle-like structure, attracting trajectories along the basin boundary and repelling them toward either the laminar or turbulent state, known as \textit{edge states}.
...
Time-averaging of this edge state revealed a close resemblance to the unstable lower-branch travelling-wave solutions, suggesting that the boundary separating the basins of attraction between the laminar and turbulent states consists of the lower branch solutions and their symmetries \citep{duguet_transition_2008,pringle_highly_2009}.
...
A graphical representation of the edge, and edge states, separating the laminar and turbulent states is shown in figure \ref{fig:EdgeStates}.
...
In particular, the solutions obtained using VWI theory by \citet{hall_streamwise_2010} are analogous to the lower branch solution from \citet{wang_lower_2007}.
...
One of the key results from this analysis is the Busse balloon, which describes the stability boundaries of ISRs as a function of $Ra$ and $Pr$, and roll wavenumber, $q$, shown in figure \ref{fig:busseballon} \citep{busse1985}.
...
These results were later verified in numerical simulations and bifurcation analysis, revealing up to twelve stable branches near the onset ($Ra \leq 2500$) and the potential for hundreds more as $Ra$ is increased \citep{ma_multiplicity_2006,boronska_extreme_2010,boronska_extreme_2010-1}.
...
This is consistent with the leading Lyapunov exponents of SDC, which become smaller with decreasing $\Gamma$ \citep{egolf_mechanisms_2000, paul_extensive_2007}.
To better characterise SDC, several studies have investigated its spatial-temporal properties, such as the averaged roll-curvature \citet{hu_convection_1995}, probability distribution of spirals \citep{ecke_excitation_1995, liu_spiral-defect_1996} and correlation length-/time-scales \citep{morris_spiral_1993, morris_spatio-temporal_1996, cakmur_transition_1997}.
...
Similar SDC-like patterns have been observed in other pattern-forming systems, including rotating RBC \citep{hu_convection_1997}, dielectric barrier discharge \citep{dong_observation_2005} and advection diffusion reaction systems \citep{affan_spiral_2014}.
...
While significant progress has been made in understanding the transition processes of Rayleigh-B\'{e}nard convection (see \S \ref{sec:bkgrd_RBC}) and plane Poiseuille flows (see \S \ref{sec:bkgrd_transitional}) independently, their combined interaction remains largely unexplored.
...
\end{enumerate}
These questions are explored in \S \ref{chap:3}.
The answers to these questions may have implications for improving microprocessing chip-cooling technologies and improving thin-film fabrication using chemical vapour deposition processes (see \S \ref{sec:bkgrd_overview}).
